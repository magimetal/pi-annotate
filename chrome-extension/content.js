/**
 * Pi Annotate - Content Script (v0.4.3)
 * 
...
(() => {
  ...
  const NOTE_VIEWPORT_MARGIN = 16;
  const PANEL_SCROLL_GUTTER = 24;
  ...
  const STYLES = `
    ...
    #pi-panel.pi-panel-collapsed .pi-panel-expanded {
      display: none;
    }

    #pi-panel.pi-panel-collapsed .pi-panel-restore {
      display: inline-flex;
    }
    ...
    .pi-panel-restore {
      ...
      border-radius: 999px;
      padding: 11px 16px;
      font: 600 12px/1 var(--pi-font-ui);
      cursor: pointer;
      white-space: nowrap;
    }

    .pi-panel-restore:hover {
    ...
    .pi-panel-restore:focus-visible,
    .pi-panel-control:focus-visible,
  ...
  function combineCssLength(baseValue, extraPx) {
    if (!extraPx) return baseValue || "";
    return baseValue ? `calc(${baseValue} + ${extraPx}px)` : `${extraPx}px`;
  }

  function capturePageStyleState() {
    if (pageStyleState || !document.body) return;
    pageStyleState = {
      root: {
      ...
      body: {
        ...
        scrollPaddingBottom: document.body.style.scrollPaddingBottom,
      }
    };
  }

  function restorePageStyleState() {
    if (!pageStyleState || !document.body) return;
    document.documentElement.style.scrollPaddingBottom = pageStyleState.root.scrollPaddingBottom;
    document.body.style.cursor = pageStyleState.body.cursor;
    document.body.style.paddingBottom = pageStyleState.body.paddingBottom;
    document.body.style.scrollPaddingBottom = pageStyleState.body.scrollPaddingBottom;
    pageStyleState = null;
  }

  function getPanelBottomInset() {
    if (!panelEl || panelCollapsed) return 0;
    return Math.ceil(panelEl.getBoundingClientRect().height);
  }

  function getNoteViewportBounds(cardSize) {
    const minX = NOTE_VIEWPORT_MARGIN;
    const minY = NOTE_VIEWPORT_MARGIN;
    const maxX = Math.max(minX, window.innerWidth - cardSize.width - NOTE_VIEWPORT_MARGIN);
    const maxY = Math.max(minY, window.innerHeight - cardSize.height - getPanelBottomInset() - NOTE_VIEWPORT_MARGIN);
    return { minX, minY, maxX, maxY };
  }

  function clampCardPosition(position, cardSize) {
    const bounds = getNoteViewportBounds(cardSize);
    return {
  ...
  function syncPageInset() {
    if (!isActive || !pageStyleState || !document.body) return;
    const inset = getPanelBottomInset();
    const scrollInset = inset ? inset + PANEL_SCROLL_GUTTER : 0;
    document.body.style.cursor = "crosshair";
    document.body.style.paddingBottom = combineCssLength(pageStyleState.body.paddingBottom, inset);
    document.documentElement.style.scrollPaddingBottom = combineCssLength(pageStyleState.root.scrollPaddingBottom, scrollInset);
    document.body.style.scrollPaddingBottom = combineCssLength(pageStyleState.body.scrollPaddingBottom, scrollInset);
  }

  function clampOpenNoteCards() {
    ...
    openNotes.forEach(index => {
      ...
      if (clamped.x !== card.offsetLeft || clamped.y !== card.offsetTop) {
        card.style.left = `${clamped.x}px`;
        card.style.top = `${clamped.y}px`;
        notePositions.set(index, clamped);
      }
    });
  }
  ...
  function syncPanelLayout() {
    ...
    const expandedPanel = panelEl.querySelector(".pi-panel-expanded");
    const restoreButton = panelEl.querySelector("#pi-restore-panel");
    if (expandedPanel) {
      expandedPanel.hidden = panelCollapsed;
      expandedPanel.setAttribute("aria-hidden", String(panelCollapsed));
    }
    if (restoreButton) {
      restoreButton.hidden = !panelCollapsed;
      restoreButton.setAttribute("aria-hidden", String(!panelCollapsed));
    }
    if (panelCollapsed) {
      elementStack = [];
      stackIndex = 0;
  ...
  function setPanelCollapsed(nextCollapsed) {
    if (!panelEl || panelCollapsed === nextCollapsed) return;
    panelCollapsed = nextCollapsed;
    syncPanelLayout();

    const focusTarget = document.getElementById(nextCollapsed ? "pi-restore-panel" : "pi-collapse-panel");
    focusTarget?.focus();
  }
  ...
  function createPanel() {
    ...
    panelEl.innerHTML = `
      <div class="pi-panel-expanded">
        <div class="pi-header">
          ...
          <div class="pi-header-actions">
            <button class="pi-panel-control" id="pi-collapse-panel" type="button" title="Collapse annotator panel" aria-label="Collapse annotator panel">−</button>
            <button class="pi-close" id="pi-close" type="button" title="Close (ESC)" aria-label="Close annotator">×</button>
          </div>
        </div>
      ...
      <button class="pi-panel-restore" id="pi-restore-panel" type="button" title="Restore annotator panel" aria-label="Restore annotator panel" hidden>
        Restore annotator
      </button>
    `;
    ...
    document.getElementById("pi-collapse-panel").addEventListener("click", () => setPanelCollapsed(true));
    document.getElementById("pi-restore-panel").addEventListener("click", () => setPanelCollapsed(false));
  ...
  function calculateNotePosition(element, cardWidth = 280, cardHeight = 150) {
    ...
    const bottomInset = getPanelBottomInset();
    const margin = NOTE_VIEWPORT_MARGIN;
    const cardSize = { width: cardWidth, height: cardHeight };
    
    // Try right side first
    if (rect.right + margin + cardWidth < vw) {
      return clampCardPosition({ x: rect.right + margin, y: Math.max(margin, rect.top) }, cardSize);
    }
    // Try left side
    if (rect.left - margin - cardWidth > 0) {
      return clampCardPosition({ x: rect.left - margin - cardWidth, y: Math.max(margin, rect.top) }, cardSize);
    }
    // Try below
    if (rect.bottom + margin + cardHeight < vh - bottomInset) {
      return clampCardPosition({ x: Math.max(margin, rect.left), y: rect.bottom + margin }, cardSize);
    }
    // Try above
    if (rect.top - margin - cardHeight > 0) {
      return clampCardPosition({ x: Math.max(margin, rect.left), y: rect.top - margin - cardHeight }, cardSize);
    }
    // Fallback: offset from element
    return clampCardPosition(
      { x: Math.min(rect.right + margin, vw - cardWidth - margin), y: Math.max(margin, rect.top) },
      cardSize
    );
  }
  ...
  function adjustForCollisions(position, cardSize, existingCards) {
    let adjusted = clampCardPosition(position, cardSize);
    const myRect = {
      left: adjusted.x,
      top: adjusted.y,
      right: adjusted.x + cardSize.width,
      bottom: adjusted.y + cardSize.height
    };
    ...
    while (attempts < 10) {
      ...
      for (const card of existingCards) {
        ...
        if (hasOverlap(myRect, cardRect)) {
          adjusted = clampCardPosition({ x: adjusted.x, y: cardRect.bottom + 12 }, cardSize);
  ...
  function createNoteCard(index) {
    ...
    if (notePositions.has(index)) {
      adjustedPos = clampCardPosition(notePositions.get(index), { width: 280, height: 150 });
      notePositions.set(index, adjustedPos);
    } else {
  ...
  function syncElementStackAtPoint(clientX, clientY, fallbackTarget = null) {
    if (!highlightEl || !tooltipEl) return null;

    const previousElement = elementStack[stackIndex] || null;
    const fallbackElement = fallbackTarget?.nodeType === Node.ELEMENT_NODE
      ? fallbackTarget
      : fallbackTarget?.parentElement || null;
    highlightEl.style.display = "none";
    tooltipEl.style.display = "none";
    const pointElement = typeof clientX === "number" && typeof clientY === "number"
      ? document.elementFromPoint(clientX, clientY)
      : null;
    const el = pointElement && !isPiElement(pointElement) ? pointElement : fallbackElement;
    highlightEl.style.display = "";
    ...
    const preservedIndex = previousElement ? nextStack.indexOf(previousElement) : -1;
    elementStack = nextStack;
    stackIndex = preservedIndex >= 0 ? preservedIndex : 0;
    return el;
  }
  ...
  function onMouseMove(e) {
    ...
    if (!syncElementStackAtPoint(e.clientX, e.clientY, e.target)) {
      hideHighlight();
      hideTooltip();
      return;
    }
  ...
  function onWheel(e) {
    ...
    if (!e.altKey) return;
    if (!syncElementStackAtPoint(e.clientX, e.clientY, e.target) || !elementStack.length) return;
  ...
  function onClick(e) {
    if (!isActive || e.target.closest("#pi-panel") || e.target.closest(".pi-note-card")) return;
    if (!syncElementStackAtPoint(e.clientX, e.clientY, e.target)) return;
  ...
  console.log("[pi-annotate] Content script ready (v0.4.3)");
})();
